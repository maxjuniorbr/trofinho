...
export default function NewTaskScreen() {
  ...
  const handleCreate = () => {
    ...
    if (Number.isNaN(pontosNum) || pontosNum <= 0)
      return setError('Pontos deve ser um número maior que zero.');
    if (pontosNum > 99999) return setError('Pontos deve ser no máximo 99.999.');
    if (selected.size === 0)
      return setError('Selecione pelo menos um filho para atribuir a tarefa.');
  ...
  const renderChildrenList = () => {
    ...
    return children.map((child) => {
      ...
      return (
        ...
        >
          <Text
            style={[styles.filhoNome, { color: sel ? colors.accent.admin : colors.text.primary }]}
          >
            {child.nome}
          </Text>
          <Text
            style={[styles.filhoCheck, { color: sel ? colors.accent.admin : colors.text.muted }]}
          >
            {sel ? '✓' : '○'}
          </Text>
        </Pressable>
