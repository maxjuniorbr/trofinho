...
import { useTransactionsByPeriod, useProfile, useMyChildId } from '@/hooks/queries';
...
export default function ChildHistoryScreen() {
    ...
    return (
        <SafeScreenFrame bottomInset>
            ...
            >
                <HeaderIconButton icon={ChevronLeft} onPress={handleBack} accessibilityLabel="Voltar" role="filho" />
                ...
                ListHeaderComponent={
                    <>
                        <View style={styles.monthNav}>
                            <Pressable onPress={goToPrevMonth} accessibilityRole="button" accessibilityLabel="Mês anterior" hitSlop={12}>
                                <ChevronLeft size={20} color={colors.text.primary} strokeWidth={2} />
                            </Pressable>
                            <Text style={[styles.monthNavLabel, { color: colors.text.primary }]}>{currentMonthLabel}</Text>
                            <Pressable onPress={goToNextMonth} accessibilityRole="button" accessibilityLabel="Próximo mês" hitSlop={12} disabled={!canGoForward}>
                                <ChevronRight size={20} color={canGoForward ? colors.text.primary : colors.text.muted} strokeWidth={2} />
                            </Pressable>
                        </View>
                        ...
                        <View style={styles.statsRow}>
                            <View style={[styles.statBox, { backgroundColor: colors.semantic.successBg }]}>
                                ...
                                <Text style={[styles.statValue, { color: colors.semantic.success }]}>+{totals.entradas}</Text>
                                <Text style={[styles.statLabel, { color: colors.semantic.successText }]}>ENTRADAS</Text>
                            </View>
                            <View style={[styles.statBox, { backgroundColor: colors.semantic.errorBg }]}>
                                ...
                                <Text style={[styles.statValue, { color: colors.semantic.error }]}>-{totals.saidas}</Text>
                        ...
                        <View style={styles.filtersRow}>
                            {FILTERS.map((f) => {
                                ...
                                if (active) {
                                    return (
                                        <LinearGradient key={f.key} colors={gradients.gold.colors} start={gradients.gold.start} end={gradients.gold.end} style={styles.filterPill}>
                                            <Pressable onPress={() => setFilter(f.key)} accessibilityRole="button" accessibilityLabel={`Filtrar ${f.label}`} style={styles.filterPressable}>
                                                <Text style={[styles.filterLabel, { color: colors.text.primary }]}>{f.label}</Text>
                                            </Pressable>
                                        </LinearGradient>
                                    );
                                }
                                return (
                                    <Pressable key={f.key} onPress={() => setFilter(f.key)} accessibilityRole="button" accessibilityLabel={`Filtrar ${f.label}`} style={[styles.filterPill, styles.filterPressable, { backgroundColor: colors.bg.muted }]}>
                                        <Text style={[styles.filterLabel, { color: colors.text.muted }]}>{f.label}</Text>
                                    </Pressable>
                                );
                            })}
                        </View>
                        ...
                        {groups.length === 0 ? (
                            <Text style={[styles.empty, { color: colors.text.muted }]}>Nenhuma transação neste mês.</Text>
                        ) : null}
                    </>
                }
...
function makeStyles(colors: ThemeColors) {
    return StyleSheet.create({
        ...
        customHeader: {
            ...
            paddingBottom: spacing['3'], borderBottomWidth: 1,
        },
        headerCenter: { flex: 1, alignItems: 'flex-start', marginHorizontal: spacing['3'] },
        headerTitle: { fontSize: typography.size.md, fontFamily: typography.family.extrabold },
        ...
        monthNavLabel: { fontSize: typography.size.md, fontFamily: typography.family.bold, textTransform: 'capitalize' },
        statsRow: { flexDirection: 'row', gap: spacing['3'], marginBottom: spacing['4'] },
        statBox: { flex: 1, borderRadius: radii.lg, borderCurve: 'continuous', paddingVertical: spacing['3'], paddingHorizontal: spacing['2'], alignItems: 'center', gap: spacing['0.5'] },
        statValue: { fontSize: typography.size.lg, fontFamily: typography.family.extrabold, fontVariant: ['tabular-nums'] },
        statLabel: { fontSize: typography.size.xxs, fontFamily: typography.family.bold, letterSpacing: 0.5 },
        filtersRow: { flexDirection: 'row', gap: spacing['2'], marginBottom: spacing['4'] },
        filterPill: { flex: 1, borderRadius: radii.full, overflow: 'hidden' },
        filterPressable: { paddingVertical: spacing['2'], alignItems: 'center', justifyContent: 'center' },
        filterLabel: { fontSize: typography.size.xs, fontFamily: typography.family.bold },
        dayBlock: { marginBottom: spacing['4'] },
        dayLabel: { fontSize: typography.size.xxs, fontFamily: typography.family.bold, letterSpacing: 0.5, marginBottom: spacing['2'] },
        dayCard: { borderRadius: radii.lg, borderCurve: 'continuous', borderWidth: 1, overflow: 'hidden' },
        txRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing['4'], paddingVertical: spacing['3'] },
        txIconBox: { width: 32, height: 32, borderRadius: radii.md, borderCurve: 'continuous', marginRight: spacing['3'] },
        txInfo: { flex: 1 },
        txLabel: { fontSize: typography.size.xs, fontFamily: typography.family.bold },
        txDesc: { fontSize: typography.size.xxs, marginTop: spacing['0.5'] },
        txSecondaryDate: { fontSize: typography.size.xxs, fontStyle: 'italic' as const, marginTop: spacing['0.5'] },
        txRight: { alignItems: 'flex-end', gap: spacing['0.5'] },
        txValue: { fontSize: typography.size.sm, fontFamily: typography.family.extrabold, fontVariant: ['tabular-nums'] },
        empty: { fontSize: typography.size.sm, textAlign: 'center', marginTop: spacing['6'] },
    });
}
