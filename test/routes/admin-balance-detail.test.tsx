...
describe('ChildBalanceAdminScreen', () => {
  ...
  it('renders a generic points management title without child photo in the header', () => {
    const renderer = render(React.createElement(ChildBalanceAdminScreen));
    const header = renderer.root.findByType('ScreenHeader' as never);
    const avatars = renderer.root.findAllByType('Avatar' as never);

    expect(header.props.title).toBe('Gestão de Pontos');
    expect(header.props.backLabel).toBe('Início');
  ...
  it('navigates back when the screen header back action is pressed', () => {
    const renderer = render(React.createElement(ChildBalanceAdminScreen));
    const pressables = renderer.root.findAll(
      (n) =>
        (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Voltar para Início',
    );
  ...
  it('opens config sheet when gear icon is pressed', () => {
    ...
    const gearBtn = renderer.root.findAll(
      (n) =>
        (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Configurar cofrinho',
    );
  ...
  it('navigates to history screen when "Ver extrato completo" is pressed', () => {
    ...
    const link = renderer.root.findAll(
      (n) =>
        (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Ver extrato completo',
    )[0];
    act(() => {
      link.props.onPress();
    });
