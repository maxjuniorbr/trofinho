...
describe('ChildBalanceHistoryScreen', () => {
  ...
  it('renders a generic statement header without child name', () => {
    const renderer = render(React.createElement(ChildBalanceHistoryScreen));
    const text = allText(renderer);
    expect(text).toContain('Extrato');
    expect(text).not.toContain('Extrato ·');
  ...
  it('filters to ganhos when pressed', () => {
    ...
    const ganhosBtn = renderer.root.findAll(
      (n) => (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Filtrar Ganhos',
    )[0];
    act(() => {
  ...
  it('filters to gastos when pressed', () => {
    ...
    const gastosBtn = renderer.root.findAll(
      (n) => (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Filtrar Gastos',
    )[0];
    act(() => {
  ...
  it('filters to cofrinho when pressed', () => {
    ...
    const cofrinhoBtn = renderer.root.findAll(
      (n) =>
        (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Filtrar Cofrinho',
    )[0];
    act(() => {
  ...
  it('navigates back when back button is pressed', () => {
    ...
    const back = renderer.root.findAll(
      (n) => (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Voltar',
    )[0];
    act(() => {
      back.props.onPress();
    });
  ...
  it('renders month navigator', () => {
    ...
    const prevBtn = renderer.root.findAll(
      (n) => (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Mês anterior',
    );
    const nextBtn = renderer.root.findAll(
      (n) => (n.type as string) === 'Pressable' && n.props.accessibilityLabel === 'Próximo mês',
    );
